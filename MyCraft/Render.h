...
public :
	...
	// Draw to screen
	void DrawScene(float3 pos, float3 dir, float dist, World* world, int look);
	void DrawSceneBlue(float3 pos, float3 dir, float dist, float time);
	...
	int FindBlock2(float3 pos, float3 dir, int depth, int3 &id, int3 &offset, int &side);
