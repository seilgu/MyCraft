...
#define MAX_SHADERS 4
	shader_program programs[MAX_SHADERS];
	...
	Shader () {
		...
		programs[DEPTH].CreateProgram("depth.vert", 0, "depth.frag");
		programs[BLUR].CreateProgram("blur.vert", 0, "blur.frag");
	}

	~Shader() {
		programs[NUL].DeleteProgram();
		programs[BLUE].DeleteProgram();
		programs[DEPTH].DeleteProgram();
		programs[BLUR].DeleteProgram();
	}

	void BuildAllPrograms();
	void UseProgram(int id);
	int UniformLocation(int program, char *name);
