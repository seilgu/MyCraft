			...
			for (int k=0; k<CHUNK_H; k++) {
				...
				if (pos.z==10)
					if (pos.x>=0 && pos.x<=30 && pos.y>=0 && pos.y<=30)
						terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
				if (pos.z==0) {
					if (pos.x>=16 && pos.x<30 && pos.y>=0 && pos.y<=30)
						terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::GLASS;
					else if (pos.x>=0 && pos.x<=15 && pos.y>=0 && pos.y<=30)
						terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
					else
						terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
				}
				...
				// text
				if (pos.x == 29 && pos.y < 18 && pos.y > 13 && pos.z > 2 && pos.z < 9) {
					terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = (s[ (pos.z-3)*4 + (pos.y-14) ] == 1 ? Block::LAVA : Block::NUL);
				}

				if (pos.x==0 || pos.x==30) {
					if (pos.y>=0 && pos.y<=30 && pos.z>=0 && pos.z<=10)
						terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
				}
				if (pos.y==0 || pos.y==30) {
					if (pos.x>=0 && pos.x<=30 && pos.z>=0 && pos.z<=10)
						terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
				}
				...
				if (pos.x == 25 && pos.y==25)
					terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
				if (pos.x == 13 && pos.y==13 && pos.z < 6)
					terrain[k*(CHUNK_W*CHUNK_L) + j*(CHUNK_W) + i] = Block::STONE;
				/*if (pos.x>0 && pos.x<30 && pos.y>0 && pos.y<30 && pos.z>0 && pos.z<30) {
					if (MersenneIRandom(0, 40) == 0)
				...
				else if (pos.z == 67 && pos.z - 1 < height[i][j]) {
